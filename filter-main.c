...
void serialFilterFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list, FILE *f  )
{
...
void parallelFilterFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list )
{
    ...
    printf ("Parallel filter first took %lu seconds and %lu microseconds.  Filter length = %d.  Num threads = %d.\n", tresult.tv_sec, tresult.tv_usec, filter_len, NUM_THREADS );
}
