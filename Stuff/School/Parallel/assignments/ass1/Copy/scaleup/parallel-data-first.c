...
void parallelDataFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, int num_threads, unsigned int* filter_list, FILE *f )
{
...
//  input_array = (unsigned int*) posix_memalign ( DATA_LEN * sizeof(unsigned int), 4096);
  ...
  for ( int num_threads =1; num_threads<=NUM_THREADS; num_threads*=2) 
  {
    ...
    parallelDataFirst ( DATA_LEN, input_array, output_array, filter_len, num_threads, filter_list, f );
